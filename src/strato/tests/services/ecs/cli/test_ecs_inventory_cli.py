...
import strato.services.ecs.cli.inventory as inventory_module
from strato.services.ecs.cli.inventory import app
from strato.services.ecs.domains.inventory.checks import ECSInventoryScanType
...
def test_inventory_scan_success(mock_run_scan):
    ...
    assert kwargs["check_type"] == ECSInventoryScanType.INVENTORY
...
def test_inventory_scan_requires_format(mock_run_scan):
    ...
    assert "Inventory data requires structured format." in (
